...
_G.Apex = nil
_G.APEX_LOADED = nil
if _G.Apex and _G.Apex.Connections then
    pcall(function()
        for _, c in pairs(_G.Apex.Connections) do
    ...
    if kind ~= "core" then
        ...
        local taskHandle = task.spawn(function()
            ...
            for _, mod in ipairs(groups[k]) do
                fetches[#fetches + 1] = HttpGet(BASE .. mod.path)
            end
            fetched[k] = fetches
        end)
        fetchTasks[#fetchTasks + 1] = taskHandle
    end
end
...
print("   Architecture: Modular (72 files)")
print("   Total Code: 2.6MB | 61,000+ Lines")
print("   Modules: 48 loaded (21 core + 7 data + 42 feature + 2 UI)")
print("========================================")
