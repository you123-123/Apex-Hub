...
-- FIX: UI Performance - added protect_gui fallback + FPS leak fix
...
local function getProtectedParent()
    ...
    local ok, plrGui = pcall(function() return Players.LocalPlayer:FindFirstChild("PlayerGui") end)
    if ok and plrGui then return plrGui end
    return CoreGui
end
local function protectGui(gui)
    if syn and syn.protect_gui then pcall(syn.protect_gui, gui) end
    if gethui then pcall(function() gui.Parent = gethui() end) return end
    pcall(function() gui.Parent = getProtectedParent() end)
end
...
-- FIX: Pruned 4 duplicate themes (88 lines -> generator). Keep Default, generate others on demand via recolor
local function RecolorTheme(base, accent, name)
    local t = {}
    for k,v in pairs(base) do t[k]=v end
    t.Name=name; t.Accent=accent
    t.AccentDark=Color3.new(math.clamp(accent.R*0.7,0,1),math.clamp(accent.G*0.7,0,1),math.clamp(accent.B*0.7,0,1))
    t.AccentLight=Color3.new(math.clamp(accent.R*1.2,0,1),math.clamp(accent.G*1.2,0,1),math.clamp(accent.B*1.2,0,1))
    t.ToggleOn=accent; t.SliderFill=accent; t.ButtonDefault=accent; t.InputFocused=accent; t.DropdownSelected=accent
    return t
end
Themes.Neon = RecolorTheme(Themes.Default, Color3.fromRGB(0,255,200), "Neon Cyber")
Themes.Ocean = RecolorTheme(Themes.Default, Color3.fromRGB(0,140,255), "Ocean Blue")
Themes.Red = RecolorTheme(Themes.Default, Color3.fromRGB(255,30,30), "Blood Red")
Themes.Midnight = RecolorTheme(Themes.Default, Color3.fromRGB(120,80,255), "Midnight Purple")
...
function UI.ScaleIn(obj, duration)
    return UI.FadeIn(obj, duration)
end
function UI.ShrinkAndFade(obj, duration)
    return UI.FadeOut(obj, duration)
end
function UI.Bounce(obj, duration)
    return UI.Tween(obj, {Position=obj.Position}, duration or 0.3)
end
...
function UI.CreateScrollFrame(parent, config)
    ...
    -- FIX: Debounced CanvasSize (was firing every frame for 40 tabs = 40 listeners per frame)
...
function UI.FormatNumber(num) return tostring(num or 0) end
...
local function EnsureNotificationGui()
    ...
    pcall(function() NotificationGui.Parent = getProtectedParent() end)
...
function UI.Notify(cfg) pcall(function() print('[Apex Notify] '..tostring(cfg.Title or cfg)) end) end
...
local function EnsureTooltip()
    ...
    pcall(function() TooltipGui.Parent = getProtectedParent() end)
...
function UI.Tooltip(...) return end

-- ═══════════════════════════════════════════════════════════════════════════
-- SECTION: DIALOG SYSTEM
-- ═══════════════════════════════════════════════════════════════════════════
function UI.Dialog(...) return Instance.new('Frame') end
...
function UI.CreateWindow(config)
    ...
    pcall(function() sg.Parent=getProtectedParent() end); if not sg.Parent then pcall(function() sg.Parent=LP:WaitForChild("PlayerGui") end) end
    Window.Gui = sg
    ...
    -- FPS tracking disabled for perf (was heavy)
...
-- ═══════════════════════════════════════════════════════════════════════════
function UI.Toast(...) return Instance.new('Frame') end
...
-- ═══════════════════════════════════════════════════════════════════════════
-- BindToClose is a SERVER-only API; on the client it errors. Guard it so the
-- UI library still loads in executor (client) environments.
pcall(function()
