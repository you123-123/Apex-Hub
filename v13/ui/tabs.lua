...
do
    ...
    Tab:AddButton({Name = "Distribute Now", Callback = function()
        if A.Stats then A.Stats.Distribute(A.Stats.GetPoints()) end
...
do
    ...
    Tab:AddButton({Name = "Start Raid", Callback = function()
        if A.Raid then A.Raid.StartRaid(A.Raid.GetRaidType()) end
...
do
    ...
    Tab:AddSlider({Name = "ESP Range [0-5000] Distinctive 3D", Flag = "ESPRange", Min = 0, Max = 5000, Default = 2500, Callback = function(v)
        A.C.ESPRange = v
        if A.ESP then A.ESP:SetRange(v) end
        A.Notify("ESP 3D", "Range: "..v.." studs | 3D Cube active", 1)
    end})
    -- Distinctive 3D toggles (not in any script)
    Tab:AddToggle({Name = "3D Bounding Box (8 corners) ✨", Flag = "ESP3DBox", Default = true, Callback = function(v)
        if A.ESP then A.ESP._settings.Box3D = v; A.ESP._settings.BoxStyle = v and "3D" or "Corner" end
        A.Notify("ESP 3D", v and "3D Cube ON - like Blender" or "2D Quad", 2)
    end})
    Tab:AddToggle({Name = "Skeleton X-Ray", Flag = "ESPSkeleton", Default = false, Callback = function(v)
        if A.ESP then A.ESP._settings.ShowSkeleton = v end
    end})
    Tab:AddToggle({Name = "Off-Screen Arrows", Flag = "ESPArrows", Default = true, Callback = function(v)
        if A.ESP then A.ESP._settings.ShowOffScreenArrow = v end
    end})
    Tab:AddToggle({Name = "Fruit Rarity % + Value", Flag = "ESPFruitRarity", Default = true, Callback = function(v)
        if A.ESP then A.ESP._settings.FruitShowRarity = v end
    end})
    Tab:AddToggle({Name = "Fruit Radar 3D Pulse 🔮", Flag = "FruitRadar", Default = false, Callback = function(v)
        if A.ESP then if v then A.ESP.StartFruitRadar() else A.ESP.StopFruitRadar() end end
        A.Notify("Fruit Radar", v and "3D Pulse ON - Legendary fruits pulse!" or "Radar OFF", 2)
    end})
    Tab:AddLabel("ESP Count: 0 | 3D Cube: 12 edges | Skeleton: 12 bones")
end
...
do
    ...
    Tab:AddToggle({Name = "SuperAttack", Flag = "SuperAttack", Default = false, Callback = function(v)
        A.F.SuperAttack = v
        if A.Combat then if v then A.Combat.SuperAttack(true) else A.Combat.StopAura() end end
...
-- TAB 49: New Features (HoHo/Redz parity + distinctive)
do
    local Tab = W:CreateTab({Name = "New Features ⭐", Color = Color3.fromRGB(255,215,0), Icon = "⭐"})
    local S1 = Tab:AddSection("Fruit Sniper (HoHo/Redz parity)")
    Tab:AddToggle({Name = "Fruit Sniper Hop", Flag = "FruitSniper", Default = false, Callback = function(v)
        if A.FruitSniper then if v then A.FruitSniper.Start() else A.FruitSniper.Stop() end end
    end})
    Tab:AddSlider({Name = "Hop Delay", Flag = "SniperDelay", Min = 3, Max = 30, Default = 6, Callback = function(v) if A.FruitSniper then A.FruitSniper.SetHopDelay(v) end end})
    local S2 = Tab:AddSection("WalkOnWater + Next Island + NoStun")
    Tab:AddToggle({Name = "WalkOnWater", Flag = "WalkOnWater", Default = false, Callback = function(v)
        if A.Movement then if v then A.Movement.StartWalkOnWater() else A.Movement.StopWalkOnWater() end end
    end})
    Tab:AddButton({Name = "Teleport: Next Island", Callback = function() if A.Teleport and A.Teleport.AutoNextIsland then A.Teleport.AutoNextIsland() end end})
    Tab:AddToggle({Name = "NoStun", Flag = "NoStun", Default = false, Callback = function(v)
        if A.AC then if v then A.AC.EnableNoStun() else A.AC.DisableNoStun() end end
    end})
    local S3 = Tab:AddSection("Mastery 5 Modes")
    Tab:AddButton({Name = "Farm Bone Mastery", Callback = function() if A.Mastery then A.Mastery.StartBone() end end})
    Tab:AddButton({Name = "Farm Gun Mastery", Callback = function() if A.Mastery then A.Mastery.StartGun() end end})
    Tab:AddButton({Name = "Farm Observation", Callback = function() if A.Mastery then A.Mastery.StartObservation() end end})
    Tab:AddButton({Name = "Farm Superhuman", Callback = function() if A.Mastery then A.Mastery.StartSuperhuman() end end})
    Tab:AddButton({Name = "Farm Death Step", Callback = function() if A.Mastery then A.Mastery.StartDeathStep() end end})
    local S4 = Tab:AddSection("Aimbot + Chest Filter + Race V4 Full")
    Tab:AddToggle({Name = "Aimbot Skill", Flag = "Aimbot", Default = false, Callback = function(v) A.F.Aimbot = v end})
    Tab:AddToggle({Name = "Chest Golden Only", Flag = "ChestGolden", Default = true, Callback = function(v) if A.Collection then A.Collection.SetChestFilter("Golden", v) end end})
    Tab:AddButton({Name = "Race V4 Full (All Trials)", Callback = function() if A.RaceV4 and A.RaceV4.StartFullRaceV4 then A.RaceV4.StartFullRaceV4() end end})
end

print("[Apex] All 49 tabs loaded! (48 + 1 New Features)")
