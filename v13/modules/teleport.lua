...
function Teleport.ToPortal()
    local myHRP = A.HRP()
    if not myHRP then return false end
    for _, child in ipairs(_SafeDescendants(Workspace,300)) do
...
-- Distinctive Auto Next Island (was missing vs HoHo) - uses autofarm GetBestZone
function Teleport.AutoNextIsland()
    local ok, zone = pcall(function()
        if A.Farm and A.Farm.GetBestZone then return A.Farm.GetBestZone() end
        if A.Islands and A.Islands.All and #A.Islands.All>0 then return A.Islands.All[1] end
    end)
    if ok and zone and zone.pos then
        return A.TP and A.TP.TPTo and A.TP.TPTo(zone.pos)
    elseif ok and zone and zone.Position then
        local pos = zone.Position.Position or zone.Position
        return A.TP and A.TP.TPTo and A.TP.TPTo(Vector3.new(pos.X, pos.Y, pos.Z))
    end
    return false
end
