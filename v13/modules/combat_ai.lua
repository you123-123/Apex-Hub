...
function CA.ComboEngine.ExecuteCombo(preset, target)
    ...
    for i, skillKey in ipairs(p.Skills) do
        ...
        task.wait(math.max(timing + jitter, 0.03))
...
function CA.ComboEngine.GetComboDamage(preset)
    ...
    local totalDmg = 0
    local dataVals = A.LP and A.LP:FindFirstChild("Data") and A.LP.Data:FindFirstChild("Values")
    local fruitStats = dataVals and dataVals:FindFirstChild("FruitStats")
...
function CA.FightWithAI(target)
    ...
    local myHealth = A.Hum() and A.Hum().Health / math.max(A.Hum().MaxHealth, 1) or 1
    local theirHealth = hum.Health / math.max(hum.MaxHealth, 1)
    local flagGet = function(key) return A.F[key] ~= false end
    if flagGet("AntiCombo") and CA.AntiCombo.DetectCombo(target) and CA.ComboBreakerReady then
        CA.AntiCombo.BreakCombo()
        return
    end
    if flagGet("PredictionCombat") then
        CA.PredictionEngine.UpdatePredictionModel(target)
        local predictedAction = CA.PredictionEngine.PredictNextAction(target)
        if predictedAction == "Attack" and flagGet("SmartDodge") then
    ...
    if flagGet("AdaptiveCombo") then
        ...
        if CA.ComboPresets[optimalPreset] and CA.ComboPresets[optimalPreset].Positioning == "Far" then
            gapCloseDist = 60
        end
        ...
        local fallback = CA.ComboEngine.OptimalCombo(target, dist, theirHealth)
        if dist > 30 then
            A.TweenTo(theirHRP.Position, 200)
            task.wait(0.1)
        else
            CA.ComboEngine.ExecuteCombo(fallback, target)
        end
    end
    if flagGet("AutoCounter") then
