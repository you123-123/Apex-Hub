...
function A.Raid.NavigateRaid()
    ...
    for _, enemy in ipairs(enemies) do
        ...
        if enemy.model and enemy.model:FindFirstChild("HumanoidRootPart") then
            local _hrpRaidDist = A.HRP()
            local dist = _hrpRaidDist and (enemy.model.HumanoidRootPart.Position - _hrpRaidDist.Position).Magnitude or math.huge
            if dist > 50 then
...
function A.Raid.SpeedRaid()
    ...
    for _, enemy in ipairs(enemies) do
        ...
        if enemy.model and enemy.model:FindFirstChild("HumanoidRootPart") then
            ...
            local _hrpRaid = A.HRP()
            if _hrpRaid then _hrpRaid.CFrame = A.CF(enemy.model.HumanoidRootPart.Position + A.V3(0, 10, 0)) end
            task.wait(0.1)
            A.SuperAttack(enemy.model)
            task.wait(0.1)
        end
