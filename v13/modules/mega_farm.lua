...
-- FIX: GetDescendants throttle helper max 300
local _MF_DescThrottle = {lastTick=0, cache=nil}
local function _MF_SafeDescendants(parent, limit)
    limit = limit or 300
    if _MF_DescThrottle.cache and (tick()-_MF_DescThrottle.lastTick)<0.5 then
        if #_MF_DescThrottle.cache <= limit then return _MF_DescThrottle.cache end
        local t={}; for i=1,limit do t[i]=_MF_DescThrottle.cache[i] end; return t
    end
    local all = parent:GetDescendants()
    local out={}; for i=1, math.min(#all, limit) do out[i]=all[i] end
    _MF_DescThrottle.cache=out; _MF_DescThrottle.lastTick=tick()
    return out
end
...
function MF.UltraFarm.UltraAutoFarm()
    ...
    if target then
        ...
        if targetHRP and targetHum and targetHum.Health > 0 then
            ...
            if dist > 50 then
                A.TweenTo(targetHRP.Position, 250)
            else
                local weapon = A.GetTool("Sword") or A.GetTool("Melee")
                if not weapon and LP.Character then
                    weapon = LP.Character:FindFirstChildOfClass("Tool")
                end
                if weapon then
                    A.EquipTool(weapon)
                end
...
function MF.FarmAllZones.ZoneOptimization(zone)
    ...
    local totalDist = 0
    local spawnPoints = {}
    -- FIX: throttle GetDescendants max 300
    for _, obj in pairs(_MF_SafeDescendants(Workspace, 300)) do
...
function MF.MegaCombo.MaxDPS(target)
    ...
    local startHealth = target:FindFirstChild("Humanoid") and target.Humanoid.Health or 0
    MF.MegaCombo.MegaComboKill(target)
    local endHealth = target:FindFirstChild("Humanoid") and target.Humanoid.Health or 0
...
function MF.FindQuestEnemy()
    ...
    if questData and typeof(questData) == "Instance" then
...
function MF.FarmQuestEnemies(quest)
    ...
    while MF.Active and attempts < 200 do
        local questActive = A.HasQuest()
        if not questActive then break end
        local target = A.FindTarget(200)
        if target then
            local targetHRP = target:FindFirstChild("HumanoidRootPart")
            if targetHRP then
                local dist = (targetHRP.Position - A.GetPosition()).Magnitude
                if dist > 40 then
                    A.TweenTo(targetHRP.Position, 250)
                else
                    MF.MegaCombo.MegaComboKill(target)
...
-- ════════════════════════════════════════════════════════════════════
-- ZONE OPTIMIZER (محلّل أفضل مناطق الزراعة)
-- يقيّم المناطق بمجموعة عوامل: كثافة الوحوش، المستوى المطلوب، وبعد
-- عن مستوانا، ويؤرّخ النتيجة كي تبثّ أفضل مسار للمزرعة.
-- ════════════════════════════════════════════════════════════════════
MF.Zones = MF.Zones or {}
MF._ZoneScores = {}
MF._ZoneEvaluated = tick()

-- تقييم منطقة: أعلى نقاط = الأنسب للزراعة
function MF.EvaluateZone(zone)
    if not zone then return -math.huge end
    local density = tonumber(zone.Density) or 1
    local levelReq = tonumber(zone.MinLv) or 1
    local myLevel = (A.Player and A.Player.Level) or MF._Level or 100

    -- كثافة (أعلى = أفضل): أثقل على الزن
    local wDensity = 0.45
    -- مستوى مناسب (قريب لمستوى اللاعب = أسرع تصفية): أثقل متوسط
    local wLevel = 0.35
    -- تعديل بسيط للمسافة/الكلفة
    local wCost = 0.20

    -- تطبيع الكثافة في مدى منطقي
    local densityTerm = math.min(1, density / 15)
    -- قرب المستوى: mark فارغ عن target
    local levelGap = math.abs(levelReq - myLevel) / math.max(1, myLevel)
    local levelTerm = math.max(0, 1 - levelGap)

    local score = densityTerm * wDensity + levelTerm * wLevel
    -- عقوبة تكلفة تنقيلة اختيارية إن توفرت بيانات
    local cost = zone.Cost or 0
    score = score - (cost / 1000) * wCost
    return score
end

-- اختيار أفضل منطقة لمستوانا الحالي (يردّ الجدول والنتيجة)
function MF.BestZone()
    local best, bestScore = nil, -math.huge
    for _, z in pairs(MF.Zones) do
        local s = MF.EvaluateZone(z)
        if s > bestScore then
            best, bestScore = z, s
        end
    end
...
-- ذاكرة التباينات: يخزن فهرس أفضل منطقة ليحسب دالة المزرعة
function MF.SetOptimalZoneForLevel(level)
    local best, score = MF.BestZone()
    if best and score > -math.huge then
        MF._OptimizedZone = best
        return best.Name or "?"
    end
    return nil
end

-- مدير جدولة: يقرر متى يبثّ تحديث الأمثلية (مؤقّت ثابت)
MF._schedOn = false
function MF.ScheduleOptimization(interval)
    if MF._schedOn then return end
    MF._schedOn = true
    interval = interval or 60
    task.spawn(function()
        while MF._schedOn do
            task.wait(interval)
            if MF._FarmAllZones then
                MF.SetOptimalZoneForLevel((A.Player and A.Player.Level) or MF._Level or 100)
            end
        end
    end)
end
function MF.StopScheduleOptimization()
    MF._schedOn = false
end
