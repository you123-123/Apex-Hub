...
A.Farm.ZoneMobs = {}
-- Distinctive Hologram Assistant (not in any script) - small 3D viewport showing farm status
A.Farm.HologramGui = nil
A.Farm.HologramViewport = nil
function A.Farm.ShowHologram(text, mobName)
    pcall(function()
        if not A.Farm.HologramGui then
            local sg = Instance.new("ScreenGui")
            sg.Name = "ApexHologram"
            ...
            local stroke = Instance.new("UIStroke", frame); stroke.Color = Color3.fromRGB(100,80,255); stroke.Thickness = 1.5; stroke.Transparency = 0.3
            frame.Parent = sg
            -- Logical hologram: text only (was Viewport 3D heavy)
            ...
            txt.Parent = frame
            A.Farm.HologramGui = sg
                    end
                    cam.CFrame = CFrame.new(Vector3.new(0,2,8), Vector3.new(0,1,0))
                    -- Slow rotation
        end
        local txt = A.Farm.HologramGui and A.Farm.HologramGui:FindFirstChild("Frame") and A.Farm.HologramGui.Frame:FindFirstChild("HoloText")
        if txt then txt.Text = "🤖 "..(text or "Farming...")..(mobName and "\n👾 "..mobName or "").."\n⏱️ "..math.floor((tick()-A.Farm.StartTime)/60).."m | Kills: "..A.Farm.KillCount end
        if A.Farm.HologramGui then A.Farm.HologramGui.Enabled = A.Farm.Active end
    end)
end
function A.Farm.HideHologram() pcall(function() if A.Farm.HologramGui then A.Farm.HologramGui.Enabled = false end end) end
A.Farm.FarmingStats = {
    kills = 0,
    quests = 0,
    xp = 0,
...
-- FIX: Data-driven zones (was 30 hardcoded Vector3 that break on map update) -> delegates to data/islands.lua
A.Farm.GetZones = function()
    -- Try data-driven first
    if _G.Apex and _G.Apex.Islands and _G.Apex.Islands.All then
        local level = getLevel()
        local sea = getSea()
        local zones = {}
        for _, isl in ipairs(_G.Apex.Islands.All) do
            local minL = isl.Level or isl.minLevel or 1
            local maxL = isl.MaxLevel or (minL + 50)
            local islSea = isl.Sea or isl.sea or 1
            local pos = isl.Position and isl.Position.Position or isl.pos
            if not pos and isl.Position then pos = Vector3.new(isl.Position.X, isl.Position.Y, isl.Position.Z) end
            if level >= minL and level <= maxL and islSea == sea then
                table.insert(zones, {name=isl.Name or isl.name, minLevel=minL, maxLevel=maxL, sea=islSea, pos=pos or A.V3(0,10,0), data=isl})
            end
        end
