...
-- ==================== v13.1 SERVER FILTER EXPANSIONS ====================

Server.MaxAgeSeconds = 0
Server.MinPlayerCount = 0
Server.MaxPlayerCount = 28
Server.RegionFilter = "any"
Server.PreferEmpty = false
Server._regionFilter = nil
Server._ageFilterEnabled = false

function Server.GetServerAgeSeconds(server)
    -- Server names for Roblox public server listings can encode creation age; fall back to a heuristic.
    if not server or not server.Name then return 0 end
    local name = server.Name
    if string.find(name, "-") then
        -- Some listing names carry a UUID; no age available. Return 0 (treat as unknown/new).
        return 0
    end
    return 0
end

function Server.SetAgeFilter(maxMinutes)
    Server.MaxAgeSeconds = math.max(0, tonumber(maxMinutes or 0) * 60)
    Server._ageFilterEnabled = Server.MaxAgeSeconds > 0
    A.Notify("Server", "Age filter set to " .. tostring(maxMinutes or 0) .. " min", 2)
end

function Server.SetMinPlayers(minPlayers)
    Server.MinPlayerCount = math.max(0, tonumber(minPlayers or 0))
    A.Notify("Server", "Min players: " .. tostring(Server.MinPlayerCount), 2)
end

function Server.SetMaxPlayers(maxPlayers)
    Server.MaxPlayerCount = math.min(100, math.max(1, tonumber(maxPlayers or 28)))
    A.Notify("Server", "Max players: " .. tostring(Server.MaxPlayerCount), 2)
end

function Server.SetRegionFilter(region)
    Server.RegionFilter = region or "any"
    Server._regionFilter = Server.RegionFilter
    A.Notify("Server", "Region filter: " .. tostring(Server.RegionFilter), 2)
end

function Server.FindEmptyServers(limit)
    limit = limit or 5
    local servers = Server.GetServerList()
    local empties = {}
    for _, srv in ipairs(servers) do
        if (srv.Playing or 0) <= Server.MinPlayerCount then
            table.insert(empties, srv)
            if #empties >= limit then break end
        end
    end
    Server._emptyServers = empties
    return empties
end

function Server.MeetsFilters(srv)
    local age = Server.GetServerAgeSeconds(srv)
    if Server._ageFilterEnabled and age > Server.MaxAgeSeconds then
        return false
    end
    local playing = srv.Playing or 0
    if playing < Server.MinPlayerCount then
        return false
    end
    if playing > Server.MaxPlayerCount then
        return false
    end
    if Server._regionFilter and Server._regionFilter ~= "any" then
        if srv.Name and not string.find(string.lower(srv.Name), string.lower(Server._regionFilter)) then
            return false
        end
    end
    return Server.CheckServerSafety(srv)
end

function Server.GetFilteredServers(criteria)
    criteria = criteria or "balanced"
    local servers = Server.GetServerList()
    local filtered = {}
    for _, srv in ipairs(servers) do
        if Server.MeetsFilters(srv) then
            table.insert(filtered, srv)
        end
    end
    if criteria == "low" then
...
function Server.ServerHopFiltered(criteria)
    if tick() - Server._lastHopTime < Server._hopCooldown then
        A.Notify("Server Hop", "Cooldown active, please wait", 3)
        return false
    end
    ...
    local list = Server.GetFilteredServers(criteria or "low")
    local server = list[1]
    if not server then
        A.Notify("Server Hop", "No servers match filters", 3)
        return false
    end
    Server._attemptCount = Server._attemptCount + 1
    Server._lastHopTime = tick()
    ...
    A.Notify("Server Hop", "Hopping to filtered server (" .. tostring(server.Playing) .. " players)", 3)
    local ok, err = SafeCall(function()
        TeleportService:TeleportToPlaceInstance(game.PlaceId, server.Id, Players.LocalPlayer)
    end)
    if not ok then
...
function Server.HopToEmpty()
    local empties = Server.FindEmptyServers(1)
    if #empties == 0 then
        A.Notify("Server Hop", "No empty servers found", 3)
        return false
    end
    local server = empties[1]
    Server._attemptCount = Server._attemptCount + 1
    Server._lastHopTime = tick()
    Server.HopCount = Server.HopCount + 1
    A.Notify("Server Hop", "Empty server found (" .. tostring(server.Playing) .. " players)", 3)
    local ok, err = SafeCall(function()
        TeleportService:TeleportToPlaceInstance(game.PlaceId, server.Id, Players.LocalPlayer)
    end)
    return ok
