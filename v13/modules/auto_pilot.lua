...
function AP.AutoStats()
    if not AP.Active then return end
    pcall(function()
        local statPoints = A.StatPoints() or 0
...
function AP.AutoSkills()
    if not AP.Active then return end
    pcall(function()
        local skillPoints = A.Points() or 0
        if skillPoints > 0 then
            local dataVals = A.LP and A.LP:FindFirstChild("Data") and A.LP.Data:FindFirstChild("Values")
            local function dval(name)
                if not dataVals then return "" end
                local v = dataVals:FindFirstChild(name)
                return v and v.Value or ""
            end
            local fightingStyle = dval("FightingStyle")
            local sword = dval("Sword")
            local gun = dval("Gun")
...
function AP.HandleBounty()
    if not AP.Active then return end
    pcall(function()
        local myBounty = A.GetBountyValue()
...
function AP.HandleAwakening()
    if not AP.Active then return end
    pcall(function()
        local fruit = A.GetFruitName()
        if fruit ~= "" and fruit ~= "None" then
...
function AP.FindBoss(bossName)
    ...
    for _, npc in pairs(_SafeDescendants(Workspace,300)) do
