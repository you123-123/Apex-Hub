...
function Bounty.GetPlayerStats(player)
    ...
    return {
        ...
        Distance = (function() local _h=A.HRP(); return _h and (_h.Position - hrp.Position).Magnitude or math.huge end)(),
        IsVulnerable = hum.Health > 0 and hum.Health < hum.MaxHealth * 0.5,
        CanKill = bounty >= 10000,
        BountyDifference = bounty - myBounty,
        Score = bounty * (1 - hum.Health / math.max(hum.MaxHealth, 1)) / math.max((function() local _h=A.HRP(); return _h and (_h.Position - hrp.Position).Magnitude or 1 end)(), 1)
    }
end
            ...
            if Bounty.CurrentTarget then
                ...
                if not tgt.Player or not tgt.Player.Character then
                    ...
                    if not tgtHum or tgtHum.Health <= 0 then
                        ...
                        if tgtHRP then
                            local _hrpB = A.HRP()
                            local dist = _hrpB and (tgtHRP.Position - _hrpB.Position).Magnitude or math.huge
                            if dist > 2000 then
                                Bounty.CurrentTarget = nil
                                Bounty._targetSwitchCount = Bounty._targetSwitchCount + 1
