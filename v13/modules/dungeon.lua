...
local _DUN_DescThrottle = {lastTick=0, cache=nil, cacheParent=nil}
local function _DUN_SafeDescendants(parent, limit)
    limit = limit or 300
    if _DUN_DescThrottle.cache and _DUN_DescThrottle.cacheParent==parent and (tick()-_DUN_DescThrottle.lastTick)<0.5 then
        if #_DUN_DescThrottle.cache <= limit then return _DUN_DescThrottle.cache end
        local t={}; for i=1,limit do t[i]=_DUN_DescThrottle.cache[i] end; return t
    end
    local all = parent:GetDescendants()
    local out={}; for i=1, math.min(#all, limit) do out[i]=all[i] end
    _DUN_DescThrottle.cache=out; _DUN_DescThrottle.cacheParent=parent; _DUN_DescThrottle.lastTick=tick()
    return out
end
            ...
            if mob:IsA("Model") then
                ...
                if hum and hrp and hum.Health > 0 then
                    table.insert(enemies, {
                        ...
                        Name = mob.Name,
                        Distance = (function() local _hrp=A.HRP(); return _hrp and (_hrp.Position - hrp.Position).Magnitude or math.huge end)()
                    })
                end
            end
        end
    end
    -- FIX: GetDescendants throttle max 300 + cache
    for _, child in ipairs(_DUN_SafeDescendants(dungeon, 300)) do
...
function Dungeon.SolvePuzzle()
    ...
    -- FIX: GetDescendants throttle max 300 + cache
    for _, child in ipairs(_DUN_SafeDescendants(puzzle, 300)) do
