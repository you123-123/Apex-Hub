...
Trading.AutoAddFruits = false
Trading.ScamDetect = true

function Trading.GetItemValues()
    local values = {}
    for k, v in pairs(HIGH_VALUE_ITEMS) do
        values[k] = v
    end
    return values
end

function Trading.GetTradeValueOfItem(itemName)
    if not itemName then return 0 end
    local raw = tostring(itemName)
    for k, v in pairs(HIGH_VALUE_ITEMS) do
        if string.find(string.lower(raw), string.lower(k)) then
            return v
        end
    end
    if A.Collection and A.Collection.GetFruitValue then
        return A.Collection.GetFruitValue(raw)
    end
    return 0
end

function Trading.CalculateTradeValue(lhsItems, rhsItems)
    local lhsTotal = 0
    local lhsBreakdown = {}
    if lhsItems then
        for _, item in ipairs(lhsItems) do
            local v = Trading.GetTradeValueOfItem(item)
            lhsTotal = lhsTotal + v
            table.insert(lhsBreakdown, {Name = tostring(item), Value = v})
        end
    end
    local rhsTotal = 0
    local rhsBreakdown = {}
    if rhsItems then
        for _, item in ipairs(rhsItems) do
            local v = Trading.GetTradeValueOfItem(item)
            rhsTotal = rhsTotal + v
            table.insert(rhsBreakdown, {Name = tostring(item), Value = v})
        end
    end
    local diff = lhsTotal - rhsTotal
    return {
        LhsTotal = lhsTotal,
        RhsTotal = rhsTotal,
        Difference = diff,
        LhsBreakdown = lhsBreakdown,
        RhsBreakdown = rhsBreakdown,
        Fair = math.abs(diff) <= math.max(20000, lhsTotal * 0.2),
        Ratio = rhsTotal > 0 and (lhsTotal / rhsTotal) or 0
    }
end

function Trading.AddFruitToTrade(fruitName)
    local ok, err = SafeCall(function()
        local remotes = ReplicatedStorage:FindFirstChild("Remotes")
        if remotes then
            local addRemote = remotes:FindFirstChild("AddTradeItem") or remotes:FindFirstChild("TradeAdd") or remotes:FindFirstChild("AddItem")
            if addRemote then
                addRemote:FireServer(fruitName)
                return true
            end
        end
        local commF = A.CommF
        if commF and commF.FireServer then
            commF:FireServer("AddTradeItem", fruitName)
            return true
        end
    end)
    return ok
...
function Trading.AutoAddAllFruits()
    local lp = A.LP
    ...
    for _, container in ipairs(containers) do
        if container then
            for _, item in ipairs(container:GetChildren()) do
                ...
                if isFruit then
                    local addedOk = Trading.AddFruitToTrade(item.Name)
                    if addedOk then
                        added = added + 1
                        task.wait(0.3)
                    end
    ...
    A.Notify("Trading", "Added " .. tostring(added) .. " fruits to trade", 3)
    return added
end
