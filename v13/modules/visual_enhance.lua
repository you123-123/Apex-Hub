...
local _VIS_DescThrottle = {lastTick=0, cache=nil}
local function _VIS_SafeDescendants(parent, limit)
    limit = limit or 300
    if not parent then return {} end
    if _VIS_DescThrottle.cache and (tick()-_VIS_DescThrottle.lastTick)<0.5 then
        if #_VIS_DescThrottle.cache <= limit then return _VIS_DescThrottle.cache end
        local t={}; for i=1,limit do t[i]=_VIS_DescThrottle.cache[i] end; return t
    end
    local all = parent:GetDescendants()
    local out={}; for i=1, math.min(#all, limit) do out[i]=all[i] end
    _VIS_DescThrottle.cache=out; _VIS_DescThrottle.lastTick=tick()
    return out
end
...
function A.Visual.DisableFog()
	...
	pcall(function()
        ...
        for _, v in ipairs(_VIS_SafeDescendants(Lighting, 300)) do
...
function A.Visual.DisableEffects()
	...
	pcall(function()
		-- FIX: throttle GetDescendants max 300
        for _, desc in ipairs(_VIS_SafeDescendants(Workspace, 300)) do
...
function A.Visual.DisableRendering()
	...
	pcall(function()
		-- FIX: throttle GetDescendants max 300
        for _, part in ipairs(_VIS_SafeDescendants(Workspace, 300)) do
...
function A.Visual.MinimalRender()
	...
	pcall(function()
		-- FIX: throttle GetDescendants max 300
        for _, obj in ipairs(_VIS_SafeDescendants(Workspace, 300)) do
...
function A.Visual.RedeemCode(code)
	...
	pcall(function()
		...
		if rs then
			...
			if remotes then
				remoteEvent = remotes:FindFirstChild("RedeemCode")
			end
		end
...
function A.Visual.BackgroundMute()
	...
	pcall(function()
		-- FIX: throttle GetDescendants max 300
        for _, desc in ipairs(_VIS_SafeDescendants(SoundService, 300)) do
