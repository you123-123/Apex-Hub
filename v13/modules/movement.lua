...
function Movement.StartFly()
    ...
    -- FIX: Stealth Fly (was huge detectable) - same as core/services.lua:1445
...
-- FIX: Throttled Noclip (was Stepped 60Hz spam detectable) - same as core/anticheat/noclip.lua:22
function Movement.StartNoclip()
    if Movement.NoclipEnabled then return end
    Movement.NoclipEnabled = true
    local lastRun = 0
    Movement._noclipConnection = RunService.Heartbeat:Connect(function()
        if not Movement.NoclipEnabled then return end
        local now = tick()
        if now - lastRun < 0.2 then return end
        lastRun = now
        local lp = A.LP
    ...
    A.Notify("Movement", "Noclip enabled [Stealth 0.2s]", 2)
end
...
-- Distinctive WalkOnWater (was hidden, now explicit Toggle vs HoHo)
Movement.WalkOnWaterEnabled = false
Movement._waterPart = nil
Movement._waterConn = nil
function Movement.StartWalkOnWater()
    if Movement.WalkOnWaterEnabled then return end
    Movement.WalkOnWaterEnabled = true
    ...
    part.Anchored = true
    part.CanCollide = true
    part.Name = "ApexWaterPlatform"
    part.Parent = workspace
    Movement._waterPart = part
    Movement._waterConn = game:GetService("RunService").Heartbeat:Connect(function()
        local hrp = A.HRP and A.HRP()
        if not hrp or not Movement.WalkOnWaterEnabled then return end
        local pos = hrp.Position
        -- Check if over water via raycast
        local ray = workspace:Raycast(pos, Vector3.new(0,-10,0), RaycastParams.new())
        if ray and ray.Material == Enum.Material.Water then
            part.CFrame = CFrame.new(Vector3.new(pos.X, ray.Position.Y + 0.5, pos.Z))
...
function Movement.StopWalkOnWater()
    Movement.WalkOnWaterEnabled = false
    if Movement._waterConn then Movement._waterConn:Disconnect(); Movement._waterConn=nil end
    if Movement._waterPart then Movement._waterPart:Destroy(); Movement._waterPart=nil end
end
