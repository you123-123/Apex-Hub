...
-- FIX: GetDescendants throttle helper (max 300, cache-aware)
local _AE_DescThrottle = {lastTick = 0, cache = nil}
local function _AE_SafeDescendants(parent, limit)
    limit = limit or 300
    -- throttle: reuse cache for 0.5s to avoid hammering workspace
    if _AE_DescThrottle.cache and (tick() - _AE_DescThrottle.lastTick) < 0.5 and _AE_DescThrottle.cache.Parent == parent then
        -- still enforce limit
        local c = _AE_DescThrottle.cache.Data
        if #c <= limit then return c end
        local t={}; for i=1,limit do t[i]=c[i] end; return t
    end
    local all = parent:GetDescendants()
    local out = {}
    for i=1, math.min(#all, limit) do out[i]=all[i] end
    _AE_DescThrottle.cache = {Parent=parent, Data=out}
    _AE_DescThrottle.lastTick = tick()
    return out
end
...
function AE.CompleteRaid()
    ...
    while AE.Active and (tick() - startTime < timeout) do
        ...
        for _, obj in pairs(_AE_SafeDescendants(Workspace, 300)) do
...
function AE.KillSeaBeast()
    ...
    while AE.Active and (tick() - startTime < timeout) do
        ...
        local seaBeast = nil
        -- FIX: throttle GetDescendants max 300 + cache
        for _, npc in pairs(_AE_SafeDescendants(Workspace, 300)) do
