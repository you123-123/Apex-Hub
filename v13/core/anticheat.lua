...
function AC.AddHistory(level, reason, time)
    ...
    local entry = {
        ...
        id = HttpService and HttpService:GenerateGUID(false) or tostring(time) .. "_" .. tostring(level)
    }
...
function AC.BypassKick()
    ...
    -- NOTE: hookmetamethod removed - conflicts with getrawmetatable hook above

    AC.AddHistory(1, "Layer 1 (Kick Bypass) activated")
end
...
function AC.UnhookKick()
    ...
    if AC._originalKick and LocalPlayer then
        pcall(function()
            LocalPlayer.Kick = AC._originalKick
        end)
    end
        ...
        if mt and mt.__namecall then
            ...
            mt.__namecall = newcclosure(function(self, ...)
                ...
                if method == "SetPrimaryPartCFrame" or method == "PivotTo" then
                    local now = GetTimestamp()
                    local elapsed = now - AC._lastTeleportTime
                    if elapsed < AC._minTeleportInterval then
                        local delay = AC._minTeleportInterval - elapsed
                        task.wait(delay)
                    end
...
function AC.BypassFlight()
    ...
    AC._flightMonitor = RunService.Heartbeat:Connect(function(dt)
        ...
        local heightVariation = math.sin(now * 0.5) * 0.1
        local jitter = Vector3.new(
            math.sin(now * 2.3) * 0.02,
            heightVariation,
            math.cos(now * 1.7) * 0.02
        )
...
function AC.BypassNoclip()
    ...
    AC._noclipHeartbeat = RunService.Stepped:Connect(function()
    ...
    AC.AddHistory(1, "Layer 6 (Noclip Bypass) activated")
end
...
function AC.HookMetamethod(name, hook)
    ...
    pcall(function()
        ...
        if mt and mt[name] then
            ...
            elseif isIndex then
                mt[name] = newcclosure(function(self, key)
                    return hook(original, self, key)
                end)
            elseif isNewIndex then
                mt[name] = newcclosure(function(self, key, value)
                    return hook(original, self, key, value)
                end)
            else
            ...
            AC.HookedMetamethods[name] = {
                original = original,
                hookedAt = GetTimestamp()
            }
    ...
    AC.AddHistory(1, "Failed to hook metamethod: " .. name)
    return false
end
...
---------------------------------------------------------------------------
-- AUTO-INITIALIZE: DISABLED - user must toggle from UI
-- pcall(function() AC.Initialize() end)
