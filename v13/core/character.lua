...
-- FIX: Wait for LocalPlayer (was nil on some executors causing kick/crash at CHARACTER)
if not LocalPlayer then
    ...
    if not LocalPlayer then
        warn("[Apex Character] LocalPlayer still nil after wait - delaying init")
        task.wait(2)
        LocalPlayer = Players.LocalPlayer
    end
end
...
function A.IsPirate()
    ...
    return name == "Pirates" or name == "Pirate"
end

function A.IsMarine()
    local name = A.TeamName()
    return name == "Marines" or name == "Marine"
end
