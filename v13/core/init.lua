...
if _G.Apex ~= nil then
    warn("[APEX CORE] Detected existing _G.Apex instance. Attempting graceful takeover...")
    if type(_G.Apex) == "table" and _G.Apex._INITIALIZING then
        warn("[APEX CORE] Another initialization is currently in progress. Aborting.")
        return
    end
    if type(_G.Apex) == "table" and _G.Apex.Version then
        warn("[APEX CORE] Existing Apex Hub v" .. tostring(_G.Apex.Version) .. " detected.")
    end
    -- Proceed with reinitialization but log the event
end
...
-- Prevent double-init of core namespace
if _G.Apex ~= nil and type(_G.Apex) == "table" then
    -- Save reference to old instance for potential migration
    local _OLD_APEX = _G.Apex
    _OLD_APEX._DEPRECATED = true
    _OLD_APEX._REPLACED_AT = tick()
end
...
local ApexMeta = {
    __index = function(self, key)
        -- Allow access to protected keys without error
        if key == "_INITIALIZING" then
            return rawget(self, "_INITIALIZING") or false
        end
        if key == "_LOADED" then
            return rawget(self, "_LOADED") or false
        end
        if key == "Version" or key == "Build" or key == "EXEC" then
            return rawget(self, key)
        end

        -- Check if the module system has registered this key
        ...
        -- Allow standard keys
        local allowed = {
            "LP", "G", "CF", "V3", "Executor", "ExecutorData",
            "CRASH_SAFE", "SESSION_START", "SESSION_ID",
            "Log", "Logs", "LogsMutex", "Init", "GetStats",
            "SafeExec", "PerfStart", "PerfEnd", "Register",
            "GetModule", "IsLoaded", "GetLoadOrder",
            "RegisteredModules", "LoadOrder", "ErrorCodes",
            "DeepEqual", "TableMerge", "DeepCopy",
            "Environment", "Flags", "Signals",
            "GetSessionUptime", "GetExecutor", "GetExecutorInfo",
            "Fingerprint", "Subsystems", "SubsystemOrder",
        }
        ...
        -- Return nil for unknown keys instead of erroring
        return nil
    end,
    ...
    __newindex = function(self, key, value)
        ...
        local protected = {
            "Version", "LP", "G", "CF", "V3",
            "SESSION_START", "SESSION_ID", "EXEC",
        }

        if rawget(self, "_LOADED") == true then
            for _, v in ipairs(protected) do
                if key == v then
                    warn("[APEX CORE] Attempt to modify protected field '" .. key .. "' blocked by metatable.")
                    return
                end
            end
        end
...
A.Fingerprint = {
    GameVersion = game.Version or "Unknown",
...
-- ═══════════════════════════════════════════════════════════════════
-- SECTION 8: ERROR CODES REFERENCE TABLE
-- ═══════════════════════════════════════════════════════════════════

A.ErrorCodes = {
    E000 = { code = "E000", desc = "No error", severity = "NONE" },
    E001 = { code = "E001", desc = "Failed to detect executor", severity = "WARN" },
    E002 = { code = "E002", desc = "Executor is unsupported", severity = "ERROR" },
    E003 = { code = "E003", desc = "Environment integrity check failed", severity = "ERROR" },
    E004 = { code = "E004", desc = "Multiple instances detected", severity = "WARN" },
    E005 = { code = "E005", desc = "LocalPlayer is nil", severity = "ERROR" },
    E006 = { code = "E006", desc = "Game service unavailable", severity = "ERROR" },
    E007 = { code = "E007", desc = "Module registration failed", severity = "WARN" },
    E008 = { code = "E008", desc = "Module not found", severity = "WARN" },
    E009 = { code = "E009", desc = "Module already registered", severity = "WARN" },
    E010 = { code = "E010", desc = "Subsystem initialization failed", severity = "ERROR" },
    E011 = { code = "E011", desc = "UI framework failed to initialize", severity = "ERROR" },
    E012 = { code = "E012", desc = "Signal system error", severity = "ERROR" },
    E013 = { code = "E013", desc = "Performance monitor error", severity = "WARN" },
    E014 = { code = "E014", desc = "SafeExec wrapper caught exception", severity = "ERROR" },
    E015 = { code = "E015", desc = "Configuration load failed", severity = "ERROR" },
    E016 = { code = "E016", desc = "Network request failed", severity = "ERROR" },
    E017 = { code = "E017", desc = "Race condition detected", severity = "CRIT" },
    E018 = { code = "E018", desc = "Memory threshold exceeded", severity = "WARN" },
    E019 = { code = "E019", desc = "Executor capability mismatch", severity = "WARN" },
    E020 = { code = "E020", desc = "Anti-cheat detection", severity = "CRIT" },
}
...
function A.Init()
    ...
    A.Log("Step 1/9: Verifying LocalPlayer...", "INIT")
    ...
    A.Log("Step 2/9: Executor analysis complete.", "INIT")
    ...
    A.Log("Step 3/9: Environment integrity checks...", "INIT")
    ...
    if not _PRE_INIT_ENV_CHECK.hasself then
        A.Log("CRITICAL: self/global is not available!", "ERROR")
        missingCritical = true
    end
    ...
    A.Log("Step 4/9: Race condition detection...", "INIT")
    ...
    A.Log("Step 5/9: Setting core flags...", "INIT")
    ...
    A.Log("Step 6/9: Creating core signals...", "INIT")
    ...
    A.Log("Step 7/9: Initializing subsystems...", "INIT")
    ...
    -- Step 8: Post-init verification
    A.Log("Step 8/9: Post-init verification...", "INIT")
    ...
    A.Log("  Signals active:     " .. tostring((function()
        local c = 0; for _ in pairs(A.Signals) do c = c + 1 end; return c end)()), "DEBUG")
    A.Log("  Memory usage:       " .. string.format("%.2f MB", stats.memoryMB), "DEBUG")
    A.Log("  Place:              " .. tostring(stats.placeId), "DEBUG")
    A.Log("  Server Job:         " .. tostring(stats.jobId), "DEBUG")
    A.Log("Post-init verification: PASSED", "OK")

    -- Step 9: Finalization
    A.Log("Step 9/9: Finalizing initialization...", "INIT")
    A._INITIALIZING = false
    A._LOADED = true
    _G.APEX_LOADED = true
    ...
    A.Log(string.format("  Uptime: %s", stats.uptimeFormatted), "OK")
    A.Log("═══════════════════════════════════════════════════════════", "OK")
