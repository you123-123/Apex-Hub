...
-- Roblox Instances do not allow attaching custom members in modern Roblox
-- ("X is not a valid member of Y"). Wrap the real service in a proxy table
-- that (a) stores hub helper methods and (b) forwards instance API calls
-- to the real service with correct self-binding.
-- FIX: Corrected self-binding (was passing real twice when self==proxy)
local function wrapService(real)
    local proxy = {}
    setmetatable(proxy, {
...
A.WS = wrapService(A.Workspace)
A.WsCache = {}
A.WsPlayerCharacters = {}
A.WsLastCleanup = tick()

-- ════════════════════════════════════════════════════════════════════
-- EVENT BUS → مفصول إلى core/event.lua (نظام مستقل)
-- A.Event يعرَّف الآن في ملفه المخصص.
-- ════════════════════════════════════════════════════════════════════

-- =====================================================================
-- ADVANCED TASK SCHEDULER → مفصول إلى core/jobs.lua (نظام مستقل)
-- A.Jobs يعرَّف الآن في ملفه المخصص.
-- =====================================================================

-- =====================================================================
-- METRICS ENGINE → مفصول إلى core/metrics.lua (نظام مستقل)
-- A.Metrics يعرَّف الآن في ملفه المخصص.
-- =====================================================================

-- =====================================================================
-- CACHE COHERENCY MANAGER (مدير تماسك الحِفظ)
-- يحافظ على تحديث A.WsCache ويمكّن إبطالاً موجه الآلية
-- =====================================================================
function A.WS.GetCachedEx(name, parent, timeout, minAge)
    local obj = A.WS.GetCached(name, parent, timeout)
    if not obj then return nil end
    if minAge and A.WsCache[obj:GetFullName()] then
        -- تبريد الحفظ: لا يبحث من جديد إلا بعد minAge
    end
    return obj
end

function A.WS.TrackObject(obj)
    if not obj then return end
    local key = obj:GetFullName() .. ":" .. obj.ClassName
    A.WsCache[key] = obj
end

-- FIX: throttle GetDescendants + correct Vector3 check (was type==table always false)
function A.WS.RegionSnapshot(center, radius)
    local hits = {}
    if not A.Workspace then return hits end
    for _, desc in ipairs(_SafeDescendants(A.Workspace,300)) do
        local pos = desc.Position
        if pos and typeof and typeof(pos)=="Vector3" then
            local dx = (pos.X or pos.x or 0) - (center.X or center.x or 0)
            local dy = (pos.Y or pos.y or 0) - (center.Y or center.y or 0)
            local dz = (pos.Z or pos.z or 0) - (center.Z or center.z or 0)
...
-- =====================================================================
-- SLIME-TRAIL PATHFINDER (مُسيّرُ مسار المتجاورة المعمَّق)
-- توليد تسلسل نقاط mobile نحو هدف مع تجنب تجاوز العوائق العالية
-- وسلاسة الانحناءات. يعمل بدون الاعتماد على خدمة Pathfinding إن غابت.
-- =====================================================================

-- معامل سلاسة: erp يعيد موضعاً ظهورياً بين نقطتين معistق عبر عائق
function A.SmoothStep(a, b, t)
    t = math.max(0, math.min(1, t))
    local ts = t * t * (3 - 2 * t) -- smoothstep المنحنى
    return {
        X = a.X + (b.X - a.X) * ts,
        Y = a.Y + (b.Y - a.Y) * ts,
        Z = a.Z + (b.Z - a.Z) * ts,
...
-- توليد مسار متدرج (waypoints) مع تجنب العوائق عبر انحراف جانبي
function A.GenerateWaypoints(from, to, opts)
    opts = opts or {}
    local steps = opts.steps or 10
    local jitter = opts.jitter or 1.5
    local path = {}
    for i = 0, steps do
        local t = i / steps
        local base = A.SmoothStep(from, to, t)
        -- انحراف عشوائي بسيط (سلوك بشري)
        if i > 0 and i < steps then
            base.X = base.X + (math.random() * 2 - 1) * jitter
            base.Z = base.Z + (math.random() * 2 - 1) * jitter
        end
...
-- تنقيح مسار: حذف النقاط المتشابهة/المتوسطة لإزالة التشويش
function A.SimplifyPath(path, epsilon)
    epsilon = epsilon or 0.75
    if not path or #path < 3 then return path or {} end
    local out = { path[1] }
    for i = 2, #path - 1 do
        local a, b, c = path[i-1], path[i], path[i+1]
        -- زاوية الانحراف
        local dx1 = b.X - a.X; local dz1 = b.Z - a.Z
        local dx2 = c.X - b.X; local dz2 = c.Z - b.Z
        local mag1 = math.sqrt(dx1*dx1 + dz1*dz1)
        local mag2 = math.sqrt(dx2*dx2 + dz2*dz2)
        if mag1 > 0 and mag2 > 0 then
            local dot = (dx1*dx2 + dz1*dz2) / (mag1 * mag2)
            -- إن كان الانحناء صغيراً جداً (مستقيمة تقريباً) نتجاهل النقطة
...
-- المجرِّب الأمن: يسير على المسار مع إيقاف عند الوصول أو فقدان الهدف
function A.FollowPathAsync(path, opts)
    opts = opts or {}
    if not path or #path == 0 then return false end
    local hrp = A.Workspace and game:GetService("Players").LocalPlayer
    ...
    local stopDist = opts.stopDistance or 2
    local current = 1
    local maxT = opts.timeout or 30
    local start = tick()
    while current <= #path and (tick() - start) < maxT do
        local node = path[current]
        local d = (hrp.Position and math.abs((hrp.Position.X or 0) - (node.X or 0))
                + math.abs((hrp.Position.Z or 0) - (node.Z or 0))) or 0
        if d <= stopDist then
            current = current + 1
        end
        task.wait(0.05)
...
function A.WS.GetCached(name, parent, timeout)
    ...
    if A.WsCache[cacheKey] and A.WsCache[cacheKey].Parent then
        return A.WsCache[cacheKey]
    end
    local found = A.SafeWait(parent, name, timeout)
    if found then
        A.WsCache[cacheKey] = found
    end
    return found
end
...
A.RS = wrapService(A.ReplicatedStorage)
A.RsRemoteCache = {}
A.RsAssetCache = {}

A.RemotesFolder = A.RS:FindFirstChild("Remotes") or A.WS.GetCached("Remotes", A.RS, 10)
A.CommFRemote = nil
A.CommERemote = nil
A.Assets = nil

function A.RS.Init()
    A.RemotesFolder = A.RS:FindFirstChild("Remotes") or A.SafeWait(A.RS, "Remotes", 10)
    if A.RemotesFolder then
        A.CommFRemote = A.RemotesFolder:FindFirstChild("CommF") or A.SafeWait(A.RemotesFolder, "CommF_") or A.SafeWait(A.RemotesFolder, "CommF", 5)
        A.CommERemote = A.RemotesFolder:FindFirstChild("CommE") or A.SafeWait(A.RemotesFolder, "CommE_") or A.SafeWait(A.RemotesFolder, "CommE", 5)
    end
    A.Assets = A.RS:FindFirstChild("Assets") or A.SafeWait(A.RS, "Assets", 5)
end

function A.RS.GetRemote(name)
    if A.RsRemoteCache[name] and A.RsRemoteCache[name].Parent then
        return A.RsRemoteCache[name]
    end
    if not A.RemotesFolder then
        A.RS.Init()
    end
    local remote = nil
    if A.RemotesFolder then
        remote = A.RemotesFolder:FindFirstChild(name)
    end
    if not remote then
        remote = A.SafeWait(A.RemotesFolder or A.RS, name, 5)
    end
    if remote then
        A.RsRemoteCache[name] = remote
    end
    return remote
end

function A.RS.GetCommF()
    if A.CommFRemote and A.CommFRemote.Parent then
        return A.CommFRemote
    end
    A.RS.Init()
    return A.CommFRemote
end
...
function A.RS.GetCommE()
    if A.CommERemote and A.CommERemote.Parent then
        return A.CommERemote
    end
    A.RS.Init()
    return A.CommERemote
end
...
function A.RS.WaitForRemote(name, timeout)
    ...
    local remote = A.SafeWait(A.RemotesFolder or A.RS, name, timeout)
    if remote then
        A.RsRemoteCache[name] = remote
    end
    return remote
...
function A.RS.GetAllRemotes()
    ...
    if not A.RemotesFolder then return {} end
    local remotes = {}
    local children = A.RemotesFolder:GetDescendants()
    for i = 1, #children do
        local child = children[i]
...
function A.RS.GetAsset(name, timeout)
    if A.RsAssetCache[name] then
        local cached = A.RsAssetCache[name]
        if cached and cached.Parent then
            return cached
        end
        A.RsAssetCache[name] = nil
    end
...
-- FIX: Humanized TPTo - jitter + velocity humanization to avoid anti-teleport detection
function A.TP.TPTo(pos, range)
    ...
    -- Small jitter to break exact position fingerprint (clamped + wall check)
    local jitter = Vector3.new(
        (math.random()-0.5)*0.2,
        (math.random()-0.5)*0.1,
        (math.random()-0.5)*0.2
    )
    local testPos = pos + jitter
    -- FIX: Don't jitter into wall - raycast check
    local rayOk, rayRes = pcall(function()
        local params = RaycastParams.new()
        params.FilterDescendantsInstances = {A.LocalPlayer.Character}
        params.FilterType = Enum.RaycastFilterType.Exclude
        return workspace:Raycast(pos, jitter, params)
    end)
    if not (rayOk and rayRes) then
        pos = testPos
    end
    local attempts = 0
    local maxAttempts = 5
    while attempts < maxAttempts do
        ...
        local ok, err = pcall(function()
            -- FIX: Use Tween for >30 studs to avoid velocity spike, instant only for short
            local dist = (pos - currentCF.Position).Magnitude
            -- Distinctive Time-Rewind Trail (not in any script) - leaves ghost at previous pos
            pcall(function()
                A.TP._trailHistory = A.TP._trailHistory or {}
                table.insert(A.TP._trailHistory, 1, currentCF.Position)
                if #A.TP._trailHistory > 3 then table.remove(A.TP._trailHistory) end
                for i, oldPos in ipairs(A.TP._trailHistory) do
                    local ghost = Instance.new("Part")
                    ghost.Size = Vector3.new(2,2,2)
                    ghost.Shape = Enum.PartType.Ball
                    ghost.Material = Enum.Material.Neon
                    ghost.Color = Color3.fromRGB(100+ i*30, 80, 255)
                    ghost.Transparency = 0.6 + i*0.1
                    ghost.CanCollide = false
                    ghost.Anchored = true
                    ghost.CFrame = CFrame.new(oldPos)
                    ghost.Parent = workspace
                    game:GetService("Debris"):AddItem(ghost, 1.5 - i*0.3)
                    game:GetService("TweenService"):Create(ghost, TweenInfo.new(1.5), {Transparency=1, Size=Vector3.new(0.5,0.5,0.5)}):Play()
                end
            end)
            if dist > 30 then
                A.TP.TweenTo(pos, math.random(40,60))
                task.wait(dist / 50)
            else
                hrp.CFrame = targetCF
            end
...
function A.TP.TweenTo(pos, speed)
    if not pos then return false end
    speed = speed or 50
    -- FIX: Cancel previous tween to avoid jitter stacking
    if A.TP._movementThread then
        pcall(function() task.cancel(A.TP._movementThread) end)
        A.TP._movementThread = nil
    end
    local hrp = A.TP.GetHumanoidRootPart()
    if not hrp then return false end
    ...
    local startTime = tick()
    -- Add slight speed variance to break timing fingerprint
    local speedVar = 0.9 + math.random()*0.2
    duration = duration / speedVar
...
-- FIX: Stealth FlyTo - avoid BodyVelocity huge (known signature), use CFrame Lerp + AlignPosition if available
function A.TP.FlyTo(pos, speed)
    ...
    -- Prefer stealth method: CFrame tween (undetectable) over BodyMovers
    local usePhysics = false
    -- Only use BodyMovers if AlignPosition not available and distance is large
    if (pos - hrp.Position).Magnitude > 300 then
        usePhysics = true
    end
    if not usePhysics then
        return A.TP.TweenTo(pos, speed)
    end
    -- Fallback physics path with reduced signature (limited force)
    local bodyVelocity = Instance.new("BodyVelocity")
    bodyVelocity.MaxForce = Vector3.new(4000, 4000, 4000) -- FIX: was huge (detectable)
    bodyVelocity.P = 1250
    ...
    if A.TP._movementThread then pcall(function() task.cancel(A.TP._movementThread) end) end
    A.TP._movementThread = task.spawn(function()
        while A.TP._flyActive do
            local hrpNow = A.TP.GetHumanoidRootPart()
            if not hrpNow then break end
            local dist = (pos - hrpNow.Position).Magnitude
            if dist < 3 then break end
            local dir = (pos - hrpNow.Position).Unit
            bodyVelocity.Velocity = dir * speed
            bodyGyro.CFrame = CFrame.new(hrpNow.Position, pos)
            task.wait()
        end
        A.TP.StopFly()
    end)
...
-----------------------------------------------------------------------
-- SECTION 12: PERFORMANCE MONITORING [DELEGATED]
-----------------------------------------------------------------------
-- FIX: God Object reduction - lazy delegate to metrics.lua (was duplicated Perf, order was wrong services<metrics)
A.Perf = {}
-- Lazy metatable: if Metrics loaded later, delegate to it
setmetatable(A.Perf, {
    __index = function(self, key)
        local metrics = _G.Apex and _G.Apex.Metrics
        if metrics and metrics[key] ~= nil then return metrics[key] end
        return rawget(self, key)
    end,
    __newindex = function(self, key, value)
        local metrics = _G.Apex and _G.Apex.Metrics
        if metrics and metrics[key] ~= nil then metrics[key] = value else rawset(self, key, value) end
    end
})
A.Perf._fpsHistory = {}
...
-----------------------------------------------------------------------
-- SECTION 13: EVENT BUS SYSTEM [DELEGATED]
-----------------------------------------------------------------------
-- FIX: Was duplicated God Object (was in both services.lua and core/event.lua)
if _G.Apex and _G.Apex.EventBus and _G.Apex.EventBus.On then
    A.EventBus = _G.Apex.EventBus -- use canonical
else
A.EventBus = {}
