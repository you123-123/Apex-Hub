...
-- Distinctive NoStun (was missing vs HoHo)
AC.NoStunEnabled = false
AC._noStunConn = nil
function AC.EnableNoStun()
    if AC.NoStunEnabled then return end
    AC.NoStunEnabled = true
    AC._noStunConn = game:GetService("RunService").Heartbeat:Connect(function()
        pcall(function()
    ...
    if AC.AddHistory then AC.AddHistory(0, "NoStun ENABLED") end
end
function AC.DisableNoStun()
    AC.NoStunEnabled = false
    if AC._noStunConn then AC._noStunConn:Disconnect(); AC._noStunConn=nil end
    if AC.AddHistory then AC.AddHistory(0, "NoStun DISABLED") end
end
