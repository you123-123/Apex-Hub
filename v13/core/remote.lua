...
function A.FireServer(remote, ...)
    ...
    -- FIX: Non-blocking rate limit with jitter + Governor integration (was blocking task.wait)
    local rateCheck = tick() - A.LastCallTimestamp
    if rateCheck < A.CallRateLimit then
        local waitTime = A.CallRateLimit - rateCheck
        -- Add jitter 0-0.03s to break timing fingerprint
        waitTime = waitTime + math.random()*0.03
        -- If Governor available, check if we should throttle more on high detection
        local gov = _G.Apex and (_G.Apex.Governor or _G.Apex.SignalGovernor)
        if gov and gov.ShouldThrottle and gov.ShouldThrottle() then
            waitTime = waitTime * 1.5
        end
        if waitTime > 0 and waitTime < 0.5 then -- cap to avoid long freeze
            task.wait(waitTime)
        elseif waitTime >= 0.5 then
            -- Too long, use async queue instead of blocking caller
            warn("[Apex Hub] Rate limit high ("..string.format("%.2f",waitTime).."s), skipping immediate wait")
        end
    end
...
function A.InvokeServer(remote, ...)
    ...
    -- FIX: Same jitter + Governor check as FireServer
    local rateCheck2 = tick() - A.LastCallTimestamp
    if rateCheck2 < A.CallRateLimit then
        local waitTime = A.CallRateLimit - rateCheck2 + math.random()*0.03
        local gov2 = _G.Apex and (_G.Apex.Governor or _G.Apex.SignalGovernor)
        if gov2 and gov2.ShouldThrottle and gov2.ShouldThrottle() then waitTime = waitTime*1.5 end
        if waitTime > 0 and waitTime < 0.5 then task.wait(waitTime) end
    end
    A.LastCallTimestamp = tick()
...
function A.CommF(...)
    ...
    for attempt = 1, A.MaxRetries do
        ...
        -- FIX: Jitter + Governor (was fixed wait)
        local rateCheck = tick() - A.LastCallTimestamp
        if rateCheck < A.CallRateLimit then
            local waitTime = A.CallRateLimit - rateCheck + math.random()*0.03
            local gov = _G.Apex and (_G.Apex.Governor or _G.Apex.SignalGovernor)
            if gov and gov.ShouldThrottle and gov.ShouldThrottle() then waitTime = waitTime*1.5 end
            if waitTime > 0 and waitTime < 0.5 then task.wait(waitTime) end
        end
        A.LastCallTimestamp = tick()
...
function A.HookRemote(name, preHook, postHook)
    ...
    if remote:IsA("RemoteFunction") then
        -- FIX: remote.NameCall does not exist (was nil fallback). Use game metatable orig.
        local originalNameCall = nil
        pcall(function()
            local mt = getrawmetatable(game)
            if mt and mt.__namecall then originalNameCall = mt.__namecall end
        end)
        local HM_check = _G.Apex and _G.Apex.HookManager
        if HM_check and HM_check._originals and HM_check._originals["__namecall"] then
            originalNameCall = HM_check._originals["__namecall"]
        end
        ...
        if preHook or postHook then
            hookedNameCall = newcclosure(function(self, ...)
                ...
                -- FIX: orig may be nil on some executors, fallback to direct InvokeServer
                local result
                if originalNameCall then
                    result = originalNameCall(self, unpack(args))
                else
                    -- Fallback: direct call via RemoteFunction
            ...
            -- FIX: Use HookManager chaining (was overwriting previous __namecall)
            local HM = _G.Apex and _G.Apex.HookManager
            if HM and HM.Hook then
                HM.Hook("__namecall", "Remote_"..name, function(self, orig, ...)
                    local method = getnamecallmethod()
                    if method == "InvokeServer" and self == remote then
                        local args = {...}
                        if preHook then
                            local preResult = preHook(name, args)
                            if preResult == false then return true, nil end
                            if type(preResult)=="table" then args = preResult end
                        end
                        local result = orig(self, unpack(args))
                        if postHook then
                            local postResult = postHook(name, args, result)
                            if postResult ~= nil then result = postResult end
                        end
                        return true, result
                    end
                    return false, nil
                end)
                hookedNameCall = "HookManager_Chained"
            else
        ...
        -- FIX: Was using OnServerEvent (server-side, fails silently on client) -> use OnClientEvent or __namecall for FireServer
        A.HookedRemotes[name] = {
            Remote = remote,
            Original = nil,
            Hooked = nil,
        ...
        -- Hook FireServer via HookManager for client->server monitoring
        local HM2 = _G.Apex and _G.Apex.HookManager
        if HM2 and HM2.Hook then
            HM2.Hook("__namecall", "RemoteEvent_"..name, function(self, orig, ...)
                local method = getnamecallmethod()
                if method == "FireServer" and self == remote then
                    if A.HookedRemotes[name] then A.HookedRemotes[name].CallCount += 1 end
                    if preHook then
                        local args = {...}
                        local preResult = preHook(name, args)
                        if preResult == false then return true, nil end
        ...
        -- Also monitor server->client if possible
        local conn
        pcall(function()
            if remote:IsA("RemoteEvent") and remote.OnClientEvent then
                conn = remote.OnClientEvent:Connect(function(...)
                    if A.HookedRemotes[name] then
                        A.HookedRemotes[name].CallCount += 1
                    end
                    if postHook then pcall(postHook, name, {...}, nil) end
                end)
            end
        end)
        if conn then A.HookedRemotes[name].Connection = conn end
    end
...
function A.UnhookRemote(name)
    if not name or type(name) ~= "string" then return false, "Invalid remote name" end
    if not A.HookedRemotes[name] then return false, "Remote not hooked: " .. name end
    local hookData = A.HookedRemotes[name]
    if hookData.Connection then pcall(function() hookData.Connection:Disconnect() end) end
    pcall(function()
        local HM = _G.Apex and _G.Apex.HookManager
        if HM then
            HM.Unhook("__namecall", "Remote_"..name)
            HM.Unhook("__namecall", "RemoteEvent_"..name)
        end
    end)
...
function A.AntiKick()
    ...
    -- FIX: Was hooking then immediately unhooking (dead code) - now uses HookManager chaining
    local HM = _G.Apex and _G.Apex.HookManager
    if HM and HM.Hook then
        HM.Hook("__index", "AntiKick_Remote", function(self, orig, index)
            if self == player and (index == "Kick" or index == "kick") then
                return true, function(_self, msg)
    ...
    elseif hookmetamethod then
        ...
        -- FIX: Removed immediate unhook that cancelled the protection
    end
...
function A.SafeInvokeName(name, ...)
    local remote = A.FindRemote(name)
    if not remote then
        return nil
    end
...
-- ════════════════════════════════════════════════════════════════
-- RATE GOVERNOR & TRAFFIC SHAPING (منظّم معدل الإرسال / تشكيل حزم)
-- يوزّع عمليات الإرسال عبر فترات مع إهتزاز إنساني عشوائي لتجنّب
-- نمطية القصف المكتشفة، ويحدّ ذروة عدد الاستدعاءات لكل نافذة.
-- ════════════════════════════════════════════════════════════════
--═ SIGNAL GOVERNOR → مفصول إلى core/signal_governor.lua (نظام مستقل)
--═ REMOTE MAP → مفصول إلى core/remote_map.lua (نظام مستقل)
-- A.SignalGovernor/A.SafeFire/A.SafeInvoke و A.RemoteMap بأنواعها تُعرَّف الآن في ملفيهما المخصصين.
