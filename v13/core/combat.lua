...
local A = _G.Apex or {}
A.__index = A
-- FIX: Forward declarations (was nil global call on HumanizedApproach)
local GetMyHRP; local GetCharacter; local GetHRP; local GetHumanoid
...
-- Distinctive Aimbot Skill (was missing vs HoHo) - auto aims Fire/Ice etc
function A.AimbotSkill(skillName, target)
    ...
    local predPos = hrp.Position + vel * 0.3
    -- Aim camera
    pcall(function() cam.CFrame = CFrame.new(cam.CFrame.Position, predPos) end)
    -- Fire skill via humanizer
    if A.Humanizer then pcall(function() A.Humanizer.ApplyAimOffset(predPos) end) end
    return true
end
-- FIX: Humanized combat movement - break fixed 3 stud fingerprint
local function HumanizedApproach(targetPos, baseOffset)
    baseOffset = baseOffset or A.AttackPositionOffset or 3
    ...
    dir = dir.Unit
    -- Randomize offset 2.2-4.8 and add lateral jitter 0.3-0.8
    local offset = baseOffset + (math.random()-0.5)*1.2
    local lateral = Vector3.new((math.random()-0.5)*1.0, 0, (math.random()-0.5)*1.0)
    local behindPos = targetPos + (dir * -offset) + lateral
    -- Use humanized tween for >8 studs, instant with jitter for close
    if A.TP and A.TP.TweenTo and dist > 8 then
        A.TP.TweenTo(behindPos, 45 + math.random()*15)
        task.wait(math.clamp(dist/45, 0.08, 0.35) + math.random()*0.05)
    else
        -- Small random look variance
        local lookPos = targetPos + Vector3.new((math.random()-0.5)*0.4, 0, (math.random()-0.5)*0.4)
        pcall(function() myHRP.CFrame = CFrame.new(behindPos, lookPos) end)
...
local function PerformClickAttack(targetPart)
    ...
    if not GetMyHRP() then return false end
    local targetPos = targetPart.Position
    HumanizedApproach(targetPos, 3) -- FIX: was fixed CFrame 3 studs (detectable)
...
local function PerformSkillAttack(targetPart, skillName)
    ...
    if not GetMyHRP() then return false end
    local targetPos = targetPart.Position
    HumanizedApproach(targetPos, 3) -- FIX: humanized
...
function A.Attack(target, keys, delay)
    ...
    pcall(function()
        HumanizedApproach(targetPos, A.AttackPositionOffset) -- FIX: humanized (was fixed behindPos)
    end)
        ...
        if targetHRP and myHRP then
            ...
            if dist > A.AttackRange * 1.5 then
                pcall(function()
                    ...
                    HumanizedApproach(pos, 3) -- FIX: humanized pos-3
                end)
                task.wait(0.1)
            end
        end
...
function A.OneShot(target)
    ...
    pcall(function()
        ...
        HumanizedApproach(pos, 2) -- FIX: humanized pos-2
    end)
    task.wait(0.05)
    ...
    for _, step in ipairs(oneShotSequence) do
        ...
        if tHRP and myHRP2 then
            ...
            pcall(function()
                HumanizedApproach(pos, 2) -- FIX: humanized2
            end)
            task.wait(0.03)
        ...
        while A.AutoAttackEnabled do
            ...
            if not currentTarget then
                ...
                if currentTarget then
                    ...
                    task.wait(0.5)
                    -- Continue the loop to search again instead of exiting
                end
            end
...
function A.LogKill(target, method, time)
    ...
    if A.Combat then
        A.Combat.Kills = A.Combat.Kills + 1
    end
...
---------------------------------------------------------------------------
-- Initialization
-- تعارض الدوال القديمة: A.Initialize حاكمه نسخة core/movement.lua (المستدعاة فعلاً).
-- نسخة القتال (غير مستدعاة خارجياً) أُعيدت تسميتها لتجنب الكتابة فوق الحاكم.
---------------------------------------------------------------------------

function A.InitializeCombat()
    A.ForceStop()
    A.AddBuiltInCombos()
    A.ClearBlacklist()
...
-- Combat subsystem namespace (used by the UI "Combat" tab)
A.Combat = {
    StartAura = A.StartAura,
    StopAura = A.StopAura,
    GetTarget = A.GetTarget,
    IsAuraActive = A.IsAuraActive,
    MegaCombo = A.MegaCombo,
    OneShot = A.OneShot,
    SuperAttack = A.SuperAttack,
    UltraCombo = A.UltraCombo,
    ComboAttack = A.ComboAttack,
    Attack = A.Attack,
    FindTarget = A.FindTarget,
    FindPlayerTarget = A.FindPlayerTarget,
    FindBossTarget = A.FindBossTarget,
    Kill = function(target) A.StartAutoAttack(target) end,
    Farm = function() A.StartAutoAttack(A.FindTarget(100)) end,
    Kills = 0,
}

-- ════════════════════════════════════════════════════════════════
-- NEURAL TARGETING → مفصول إلى core/neural_targeting.lua (نظام مستقل)
-- A.PredictTargetPosition / A.DesirabilityScore / A.SmartSelectTarget /
-- A.NeuralEngageBase / A.GroupTargetIDs / A.CombatBrain / A.StopCombatBrain
-- تُعرَّف الآن في ملفها المخصص (يُحمّل بعد combat).
-- ════════════════════════════════════════════════════════════════


-- (لا يُشغَّل تلقائياً — انتظار تفعيل المستخدم عبر UI/Start)

-- ════════════════════════════════════════════════════════════════
-- COMBO ORCHESTRATOR → مفصول إلى core/combo_master.lua (نظام مستقل)
-- A.ComboMaster يعرَّف الآن في ملفه المخصص.
-- ════════════════════════════════════════════════════════════════

-- ===================================================================
-- DODGE ENGINE → مفصول إلى core/dodge_engine.lua (نظام مستقل)
-- A.DodgeEngine و A.StopDodgeEngine يعرَّفان الآن في ملفهما المخصص.
-- ===================================================================
