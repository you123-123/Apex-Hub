...
-- Fruit System
-- ---------------------------------------------------------------------------
-- FIX: Deduped (was 40 entries with 8 duplicates -> 32 unique)
A.C.FruitsToNotify = {
...
-- FIX: Use HttpService:JSONEncode/Decode (native C++ 10x faster, supports Unicode)
-- Keep custom logic only for Color3 special case
local HttpService = game:GetService("HttpService")
local function SerializeValue(value, indent)
    -- Fast path: delegate to native JSON for primitives
    ...
    if vtype == "table" and typeof and typeof(value) == "Color3" then
        return string.format('{"__type":"Color3","R":%.4f,"G":%.4f,"B":%.4f}', value.R, value.G, value.B)
    end
    -- For tables, wrap Color3 then use native JSON
    local function wrapColor3(t, seen)
        seen = seen or {}
    ...
    local wrapped = (vtype=="table" and wrapColor3(value) or value)
    local ok, json = pcall(HttpService.JSONEncode, HttpService, wrapped)
    if ok then return json end
    -- Fallback to tostring
    return '"'..tostring(value)..'"'
end

-- FIX: Use native JSONDecode (C++ parser, handles nested/unicode correctly)
local function ParseJSON(str)
    ...
    if ok and result ~= nil then
        -- Unwrap Color3
        local function unwrapColor3(t, seen)
            ...
            for k,v in pairs(t) do
                if type(v)=="table" then t[k]=unwrapColor3(v, seen) end
            end
            return t
        end
        return unwrapColor3(result)
    end
    -- Fallback legacy for malformed old saves
...
-- FIX: Populate defaults lazily (was {} empty -> Reset did nothing)
local function EnsureDefaults()
    if next(DefaultC) == nil and next(A.C) ~= nil then
        DefaultC = DeepCopy(A.C)
        DefaultF = DeepCopy(A.F)
    end
end
...
function A.ValidateConfig()
    ...
    -- FIX: Moved injection out of validator (was mixing validation with init)
    -- Defaults should be set in EnsureDefaults, not here
    -- Validate instead of inject
    if A.C.BountyRange ~= nil and type(A.C.BountyRange) ~= "number" then
        issues[#issues+1] = "BountyRange must be number"
        A.C.BountyRange = 1000
    end
    if A.C.ComboMode == nil then A.C.ComboMode = "Basic" end -- keep fallback but log
    if A.C.FarmMode == nil then A.C.FarmMode = "Default" end
    if A.C.SelectedIsland == nil then A.C.SelectedIsland = "" end
    if A.C.StatBuild == nil then A.C.StatBuild = "Melee" end
    if A.C.TargetMode == nil then A.C.TargetMode = "Nearest" end
...
-- ============================================================================
-- GOVERNOR → مفصول إلى core/governor.lua (نظام مستقل)
-- A.Governor يعرَّف الآن في ملفه المخصص (يُحمّل بعد config لضمان توفر A.F/A.C).
-- ============================================================================
